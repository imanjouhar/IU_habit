...
def show_summary_current_habits():
    """Show a summary of current habits."""
    query = "SELECT HABIT_ID, NAME, START_DATE FROM HABIT ORDER BY START_DATE"
    display_data(query, formatter=lambda habit: f"Habit ID: {habit[0]}, Name: {habit[1]}, Start Date: {habit[2]}")

def show_summary_current_tasks():
    ...
    """
    display_data(query, formatter=lambda task: f"Task ID: {task[0]}, Name: {task[1]}, Status: {task[2]}, Status Date: {task[3]}, Periodicity: {task[4]}, Habit ID: {task[5]}")

def show_tasks_by_periodicity():
    ...
    """
    display_data(query, formatter=lambda row: f"Periodicity: {row[0]}, Count: {row[1]}")

def show_all_streaks():
    """Query from Task History table to show streaks for all tasks together"""
    with connect_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT task_id FROM TASK_HISTORY ORDER BY task_id")
        tasks = cursor.fetchall()
        for task in tasks:
            task_id = task[0]
            show_streaks_per_task(task_id)
            print('-----------------------------------------')
            longest_run_streak_given_task(task_id)
            print('-----------------------------------------')
            calculate_streak_for_task(task_id)
...
def longest_run_streak_all_tasks():
    ...
    with connect_db() as conn:
        ...
        for task in all_tasks:
            task_id = task[0]
            streak = calculate_streak_for_task(task_id, cursor)
...
print('Show summary current habits')
print('====================================')
show_summary_current_habits()

print('Show summary current tasks')
print('====================================')
show_summary_current_tasks()

print('Longest run streak per task:')
print('====================================')
longest_run_streak_given_task(3)

print('Calculate streak for task:')
print('====================================')
calculate_streak_for_task(1)

print('Show task by periodicitiy:')
print('====================================')
show_tasks_by_periodicity()

print('Show all streaks:')
print('====================================')
show_all_streaks()
