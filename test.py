...
def test_get_summary_current_habits():
       rows = get_summary_current_habits()
       assert len(rows) >= 5

def test_get_summary_current_tasks():
       rows = get_summary_current_tasks()
       assert len(rows) >= 5

def test_longest_run_streak_given_task():
        longest = longest_run_streak_given_task(21)
        assert longest > 0

def test_calculate_streak_for_task():
        max_streak = calculate_streak_for_task(21)
        assert max_streak >= 0

def test_get_tasks_by_periodicity():
       by_periodicity = get_tasks_by_periodicity()
       assert len(by_periodicity) == 4

# def test_get_all_streaks():

#def test_show_broken_task():

#def test_show_average_completion_current_tasks():

#def test_show_percentage_completed_per_task():

#def test_show_streaks_per_task(2):

#def test_longest_run_streak_all_tasks():
